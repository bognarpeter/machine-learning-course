...
fraud_train <- fraud.subset.norm[1:(nrow(fraud.subset.norm) * 0.2),]
fraud_test <- fraud.subset.norm[(nrow(fraud.subset.norm) * 0.2 + 1):(nrow(fraud.subset.norm) * 0.3),]
library(class)
...
fraud_pred_1 <- knn(fraud_train, fraud_test, fraud[1:(nrow(fraud.subset.norm) * 0.2),10], k=1)
end_time_1 <- Sys.time()
duration_1 <- end_time_1 - start_time_1
tab_1 <- table(fraud_pred_1, fraud[(nrow(fraud.subset.norm) * 0.2 + 1):nrow(fraud.subset.norm) * 0.3,10])
